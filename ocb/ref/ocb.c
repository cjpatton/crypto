...
#include "stdio.h"
#define PRINTBLOCK(name, block) printf("%s ", name); \
   for (int i=0; i<16; i++) printf("%02x", block[i]); printf("\n");
...
static int ocb_crypt(unsigned char *out, unsigned char *k, unsigned char *n,
    ...
    //PRINTBLOCK("lstar", lstar);
    /* L_$ = double(L_*) */
    double_block(ldollar, lstar);
    //PRINTBLOCK("ldollar", ldollar);
...
int test1() {
    ...
    ocb_encrypt(c, zeroes, nonce, zeroes, 128, p, 128);
    return 0;
}
